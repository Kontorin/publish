...
### 2. Сетевая модель данных — это
Модель представления данных в виде графа, где элементы (сущности) связаны отношениями (связями) произвольного типа, образуя сеть. Наиболее распространенный пример — ER-модель (модель «сущность-связь»), используемая при проектировании баз данных.

### 3. Отличие нисходящего проектирования от восходящего
- Нисходящее проектирование (сверху вниз): Исходная задача последовательно разбивается на более простые подзадачи до уровня элементарных операций. Проектирование начинается с общей структуры.
- Восходящее проектирование (снизу вверх): Сначала разрабатываются и тестируются элементарные модули (подалгоритмы), которые затем объединяются во всё более сложные структуры, пока не будет решена исходная задача.

### 4. Множество как структура данных — это
Неупорядоченная совокупность уникальных (неповторяющихся) элементов. Основные операции: проверка принадлежности элемента (`MEMBER`), добавление (`INSERT`), удаление (`DELETE`), а также теоретико-множественные операции — объединение (`UNION`), пересечение (`INTERSECTION`), разность (`DIFFERENCE`).

### 5. Как удаляется элемент из односвязного списка
1. Найти удаляемый элемент, отслеживая при этом указатель на предыдущий элемент.
2. Изменить указатель `Next` предыдущего элемента так, чтобы он ссылался на элемент, следующий за удаляемым.
3. Освободить память, занимаемую удаляемым элементом.

### 6. Как в таблице прямого доступа осуществляется поиск данных
Ключ поиска напрямую используется в качестве индекса (адреса) для доступа к соответствующей записи в таблице. Поиск выполняется за константное время O(1), так как не требует просмотра других элементов. Недостаток — необходимость иметь пространство ключей, соизмеримое с диапазоном их возможных значений.

### 7. Как структура дерева может быть представлена входным потоком данных

Ответ: С помощью линейной скобочной записи (линеаризации), основанной на обходе дерева. Часто используется формат, где сначала указывается узел, а затем рекурсивно его поддеревья. Пустые поддеревья (или отсутствие потомков) обозначаются специальным символом (например, `*` или `null`).